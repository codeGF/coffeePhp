...
class Upload extends Base
{
    ...
    private function _files()
    {
        ...
        if (!empty($_FILES)) {
            ...
            $this->auto->helpers->output->set(false, $this->_ercode[1]);
        }
        return;
    }
            ...
            if (is_array($this->_files["type"])) {
                foreach ($this->_files["type"] as $value) {
                    if (!preg_match("/{$type}/", $value)) {
                        $this->auto->helpers->output->set(false, $this->_ercode[2]);
                    }
                }
        ...
        if (!empty($size)) {
            if (is_array($this->_files["size"])) {
                foreach ($this->_files["size"] as $value) {
                    if ($value >= $size) {
                        $this->auto->helpers->output->set(false, $this->_ercode[3]);
                    }
                }
    ...
    private function _detectError()
    {
        if (is_array($this->_files["error"])) {
            foreach ($this->_files["error"] as $value) {
                if ($value != 0) {
                    $this->auto->helpers->output->set(false, $this->_ercode["system"][$value]);
                }
            }
        } else if ($this->_files["error"] != 0) {
            $this->auto->helpers->output->set(false, $this->_ercode["system"][$this->_files["error"]]);
        }
        return;
    }
